...
Use essa explica??o para a fun??o abaixo kkk
*/
...
cron.schedule('0 0 * * *', () => {
  ...
  writeUsersToFile(users);
  console.log('As requisi??es dos usu芍rios foram reiniciadas.');
});

// Fun??o para verificar se o usu芍rio 谷 premium
...
// Fun??o para alterar a API key do usu芍rio
...
if(hora > "00:00:00"){
var dia1 = 'Tenha uma 車tima madrugada ??' 
} 
if(hora > "05:30:00"){
var dia1 = 'Tenha um 車timo dia ?' 
}
if(hora > "12:00:00"){
var dia1 = 'Tenha uma 車tima tarde ?' 
}
if(hora > "19:00:00"){
var dia1 = 'Tenha um 車timo noite ?' 
}       
...
// Rota para exibir o formul芍rio de registro
app.get("/register", (req, res) => {
  res.render("register", { message: "" });
});

// Fun??o para gerar uma chave aleat車ria de 8 d赤gitos
...
// Rota para processar o formul芍rio de registro
  ...
  try {
    ...
    if (userExists) {
      return res.render("register", {
        message: "Esse usu芍rio j芍 existe no sistema. Tente outro nome de usu芍rio.",
      });
    }
...
const newUser = {
    ...
    console.error("Erro ao registrar novo usu芍rio:", error);
    res.status(500).send("Erro interno ao processar a solicita??o.");
  }
});
...
app.post("/perfil2", async (req, res) => {
  ...
  if (currentUserIndex !== -1) {
    // Atualizando as informa??es do usu芍rio
    users[currentUserIndex].key = key;
  
    writeUsersToFile(users);
    res.redirect("/login");
        const inf2 = "Por ventura, se n?o foi voc那 que mudou, contate algum administrador do site.";

  }
    } else {
        res.json({ error: "Voc那 n?o 谷 um usu芍rio premium." });
        res.redirect('/perfil');
    }
...
// Rota para processar o formul芍rio de altera??o de configura??es do usu芍rio
...
// Rota para processar o formul芍rio de login
...
try {
        ...
        res.status(500).json({ status: false, mensagem: "Erro interno ao processar a solicita??o." });
    }
});
...
status: 'ERRO', 
admin: 'https://wa.me/556199317165', 
resultado: 'ocorreu um erro no servidor interno, contate o admin no n迆mero acima!!'
})
});
} catch(err) {
console.log(err)
...
res.json({status: true, resultado: data, code: 200})
}).catch(e => {
res.json({status: false, code: 404, message: `Erro no Servidor Interno`})
})
})
...
var { url } = req.query;
if(!url)return res.json({status:false, message:'Cad那 o par?metro url'})
...
app.get('/api/filme', async (req, res, next) => {
  try {
    ...
    if (!nome) return res.json({ status: false, message: 'Cad那 o par?metro nome' });
    ...
    res.status(500).json({
      status: false,
      message: 'Erro ao processar a solicita??o',
      error: e.message
    });
  }
});
...
var { query } = req.query
if(!query)return res.json({status:false, message:'Cad那 o par?metro: query'})
const { username, apikey } = req.query;
if (!username || !apikey) {
return res.status(400).json({ error: "Faltou o par?metro 'username' ou 'key' na query" });}
const users = readUsersFromFile();
...
fonte = ["Days%20One","Domine","Exo","Fredoka%20One","Gentium%20Basic","Gloria%20Hallelujah","Great%20Vibes","Orbitron","PT%20Serif","Pacifico"]//FONTS N?O MEXA
...
app.get('/api/attp', async (req, res) => {
  try {
    const texto = req.query.texto;
    if (!texto) {
      return res.json({ status: false, motivo: 'Cad那 o par?metro texto?' });
    }
...
if(!texto)return res.json({status:false,motivo:'Cad那 o par?metro texto?'})
...
var texto = req.query.texto 
if(!texto)return res.json({status:false,motivo:'Cad那 o par?metro texto?'})
console.log(color(' 岫 APIKEY:'  + ` ${apikey} ? USERNAME: ${username}\n`,'red'),color('岫','red'), color('REQUEST', 'red'), '? ' + color('PING: ' + ping(), 'red'))
...
return res.json({message: "Ocorreu um erro ao gerar o gif, por favor notifique ao administrador, enviando o print da p芍gina.", errorMessage: String(e)})
})
} catch (e) {
return res.json({message: "Ocorreu um erro ao gerar o gif, por favor notifique ao administrador, enviando o print da p芍gina.", errorMessage: String(e)})
}
...
if (!frase) return res.json({ status: false, motivo: 'Cad那 o par?metro frase' });
const autor = req.query.autor;
if (!autor) return res.json({ status: false, motivo: 'Cad那 o par?metro autor' });
...
var frases = ['amor imaturo diz: Eu te amo porque preciso de voc那','A vida come?a a cada cinco minutos','Onde as palavras falham, a m迆sica fala','Um bom viajante n?o tem planos','Uma vez que aceitamos nossos limites, vamos al谷m deles','O que n?o nos mata nos fortalece','Se voc那 caminha sozinho, voc那 vai mais r芍pido Se voc那s caminharem juntos, voc那s ir?o mais longe','Uma vida cheia de erros n?o 谷 apenas mais honrosa, mas 谷 mais s芍bia do que uma vida gasta sem fazer nada','Nunca perca o senso de humor e aprenda a rir de suas pr車prias falhas','A preocupa??o 谷 como uma cadeira de balan?o, ela mant谷m voc那 ocupado, mas n?o leva a lugar nenhum','O homem que viveu mais tempo n?o 谷 aquele que completou mais anos, mas aquele que mais experimentou a vida','Se voc那 pode sonhar, voc那 pode fazer','O imposs赤vel 谷 o fantasma dos t赤midos e o ref迆gio dos covardes','O caminho que temos que percorrer 998 谷 cheio de surpresas. Voc那 nunca estar芍 preparado para aqueles que o tocam, sejam eles felizes ou sombrios, porque isso faz parte de ganhar experi那ncia. E descobrir qu?o agrad芍veis ou infelizes s?o aqueles que esperam por voc那, 谷 algo que voc那 nunca poder芍 evitar','A felicidade n?o 谷 algo que voc那 adia para o futuro, 谷 algo que voc那 projeta para o presente','Os amigos devem ser como dinheiro, que antes de precisar, voc那 sabe o seu valor','O homem que viveu mais tempo n?o 谷 aquele que completou mais anos, mas aquele que mais experimentou a vida','Felicidade 谷 s車 quest?o de ser.','Acredite: sempre tem algo bom guardado para voc那','Concentre-se no que est芍 buscando, n?o no que est芍 deixando para tr芍s.','A vida 谷 muito curta pra n?o viver sorrindo por a赤!','Onde h芍 vontade, h芍 chance de dar certo!','Dance no seu ritmo! ?','S車 voc那 sabe o que te deixar芍 feliz.','N?o se estresse com o que est芍 fora do seu controle.','Aprenda a apreciar as voltas que o mundo d芍.','Comece a se amar. O resto vir芍 depois.','Maior que a tristeza de n?o haver vencido 谷 a vergonha de n?o ter lutado!','Reciprocidade, para as coisa boas. Imunidade, para as coisas ruins.','Coragem, a vida gosta de pessoas destemidas.', 'Compartilhe seus sentimentos. Nem todas as pessoas sabem adivinhar','Continue caminhando, n?o tem problema se for devagar.','Melhor amar do que ser amargo!','N?o corrigir nossas falhas 谷 o mesmo que cometer novos erros','Quando o caminho se torna duro, s車 os duros continuam caminhando','Florescer exige passar por todas as esta??es!','Quando as coisas simples parecem especiais, voc那 percebe como a vida pode ser boa.','Os aprendizados deixam a vida especial.','Feliz daquele que encontra o verdadeiro amor sem as cicatrizes da decep??o','N?o conte os dias, viva-os! ???','Tudo que vem, vem com algum prop車sito. Assim como tudo que vai, vai por uma raz?o. ??','Eu n?o gosto de cobrar atitude de ningu谷m porque eu tenho de sobra. ?','Gostar, eu gosto de muita gente, mas a minha prioridade sempre ser芍 eu mesma. ?','As pessoas que criticam, s?o as mesmas que copiam. ?','Aprendi que meu 迆nico limite 谷 a minha mente. ??','Fazendo dos meus sonhos, um objetivo. ?']
...
if (!resultadoDiminuicao) {return res.status(400).json({ error: "Saldo insuficiente." });}
  try {
    ...
    console.error(error);
    res.status(500).json({ status: false, message: "Erro ao processar a requisi??o" });
  }
});
...
if (!link) return res.json("coloque o link do 芍udio e na URL ?");
try {
//C?DIGO DO ASSEMBLYAi
...
code: 200,
criador: `${criador}`,
Erro: "Ocorreu um erro ao transcrever o 芍udio! Por favor, tente novamente."
});}
...
code: 200,
criador: `${criador}`,
Erro: "N?o foi poss赤vel transcrever o 芍udio enviado."
});}
res.json({
status: true,
code: 200,
...
if (!ling) return res.json({status: 404, message : "par?metro de entrada: ling. Voc那 pode ver a lista de idiomas em https://cloud.google.com/translate/docs/languages"})  
...
return res.json({status: "Offline", resultado: {message: 'Ocorreu um erro ao executar 角 a??o, por favor, notifique ao administrador da p芍gina.', errorMessage: String(errorMessage)}, statusCode: 404});
});
});
...
if (!resultadoDiminuicao) {return res.status(400).json({ error: "Saldo insuficiente." });}
    ...
    gttsInstance.save(ranm, text, function() {
        exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
            if (err) {
                fs.unlinkSync(ranm);
                return res.status(500).json({ status: false, criador: `${criador}`, mensagem: "Erro ao processar o 芍udio" });
            }
...
//===================[ PHOTOOXY ]===================//
   ...
   res.status(404).json({
            status:404,
            error: 'Esta p芍gina n?o esta presente na Rest Api',
            endpoint: req.path
        })
})
